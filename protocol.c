...
void Prot_process_scat(sys_scatter *scat, int total_bytes, int mode)
{
	...
	Alarm(EXIT, "Prot_process_scat(): Got a corrupted message\n");
    ...
    else if(Is_link_state(pack_hdr->type)) {
        /*  Alarm(PRINT, "\n\nprocess_link_state_packet: size: %d\n", total_bytes); */ 
        Process_link_state_packet(pack_hdr->sender_id, scat->elements[1].buf, 
				  pack_hdr->data_len, pack_hdr->ack_len, 
				  type, mode);
	/* It's ok here, Process_link_state_packet() does not need the buffer 
	   after returning */
    }
    else {
        Alarm(EXIT, "Prot_process_scat(): Unknown message type: %X\n",
	      pack_hdr->type);
    }
}
